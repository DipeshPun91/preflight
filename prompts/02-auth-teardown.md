...
Your login page is a door. This prompt checks whether the walls around it
exist: routes that skip the session check, tokens that never expire,
password rules you left off, reset flows that let a stranger claim someone
else's account. The finding that shows up in almost every vibe-coded app: an
endpoint that trusts the user ID sent in the request instead of the one on
the session.
